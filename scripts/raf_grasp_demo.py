...
def main():
    ...
    starting_joint_angles = {'left_e0': -0.09625729443980971,
                             'left_e1': 1.1163545183836099,
                             'left_s0': 0.07171360183364309,
                             'left_s1': -1.217597250384047,
                             'left_w0': 0.014956312681882784,
                             'left_w1': 1.5251603983550726,
                             'left_w2': -0.08360195293975504}

    drop_joint_angles = {'left_e0': -0.16605342028859604,
                         'left_e1': 0.24351945007680942,
                         'left_s0': 0.04601942363656241,
                         'left_s1': -0.45674277959288195,
                         'left_w0': 0.08551942892461181,
                         'left_w1': 1.4641846620366274,
                         'left_w2': 0.09357282806101024}
    ...
    run.move_to_angles(starting_joint_angles)
    run.gripper_open()
    ...
    # while not rospy.is_shutdown():
        ...
        centroid, gripper_angle = run.compute_grasp()
        ...
        if centroid is not None:
            ...
            # print("Grasp Point :" + "[" + str(point_robot[0][0]) + ", "  + str(point_robot[1][0]) + ", " + str(point_robot[2][0]) + "]")

            print("Grasp Point " + str(i+1) + ": " + "[" + str(point_robot[0][0]) + ", "  + str(point_robot[1][0]) + ", " + str(point_robot[2][0]) + "]")

            # Rotate gripper
            joint_angles = {'left_e0': -0.09625729443980971,
                            'left_e1': 1.1163545183836099,
                            'left_s0': 0.07171360183364309,
                            'left_s1': -1.217597250384047,
                            'left_w0': 0.014956312681882784,
                            'left_w1': 1.5251603983550726,
                            'left_w2': math.radians(-1*(gripper_angle - 90))}

            run.move_to_angles(joint_angles)

            #  Move gripper to object with correct orientation
            ...
            # Move gripper and object to above user's hands
            run.move_to_angles(drop_joint_angles)
