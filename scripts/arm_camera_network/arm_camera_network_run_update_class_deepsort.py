...
class updateClasses(object):
    def __init__(self):
        ...
        self.detectron2 = Detectron2(detectron2_checkpoint="/home/labuser/ros_ws/src/odhe_ros/arm_camera_dataset/output/model_final.pth", use_cuda="True")
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        # Get detections and sort (Old Way)
        ...
        # Get detections and sort (New Way)
        bbox_xcycwh, cls_conf, cls_ids = run.detectron2.detect(img)
        ...
        if bbox_xcycwh is not None:
            ...
            outputs = run.deepsort.update(bbox_xcycwh, cls_conf, im)
            if len(outputs) > 0:
                bbox_xyxy = outputs[:, :4]
...
if __name__ == '__main__':
    ...
    # Run mouse click callback in a separate thread
    e = Clickonacci()
    e.setDaemon(True) 
    e.start()
